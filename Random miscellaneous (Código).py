...
def ContraseñaAleatoria(): #Función de contraseña aleatoria.
    x = 0 #Variable para numerar los resultados.
    amount = int(input("¿Cuantas contraseñas quieres generar? ")) #Indica el número de resultados que quieres obtener.
    for i in range(0, amount): #Bucle de incremento iterable.
...
while True: #Repite el programa siempre que lo parámetros se cumplan.
    ...
    while menu != 1 and menu != 2 and menu != 3 and menu != 4 and menu != 5 and menu != 6 and menu != 7 and menu != 8 and menu != 9 and menu != 0 and menu != 11 and menu != 99:
        print("\n-----------------------------------")
